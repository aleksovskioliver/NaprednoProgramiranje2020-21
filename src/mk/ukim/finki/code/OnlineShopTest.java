...
class ComparatorTypeClass{
    public static Comparator<Product> createComparator(COMPARATOR_TYPE comparatorType){
...
class OnlineShop {
    ...
    OnlineShop() {
        this.productByCategory = new HashMap<>();
    ...
    void addProduct(String category, String id, String name, LocalDateTime createdAt, double price){
        ...
        productByCategory.computeIfPresent(category,(k,v)->{
            v.add(product);
            return v;
        });
    }
//Product with id 1718a7753 does not exist in the online shop!
    double buyProduct(String id, int quantity) throws ProductNotFoundException{
        ...
        double price = p.buyProduct(quantity);
        return price;
    }

    List<List<Product>> listProducts(String category, COMPARATOR_TYPE comparatorType, int pageSize) {
        List<Product> products = (category==null) ? new ArrayList<>(productById.values()) : productByCategory.get(category);
        Comparator<Product> productComparator = ComparatorTypeClass.createComparator(comparatorType);
